...
class PANDA_IMAGE:
    def __init__(self, basepath, annofile, annomode, addmode,savepath,showcate,extraannofile=None, showwidth=1024):
        """
    ...
    def showAnns(self,imgrequest=None, range=10, imgfilters=[], shuffle=True, saveimg=False):
        ...
        :return:
        """
        savedir =os.path.join(self.basepath, self.savepath)
        ...
        for imgname in imgnames:
            ...
            elif self.annomode =='crowd':
                print(self.annomode)
                imgwithann = self._addCrowdAnns(imgname, img)
            elif self.annomode =='people':
                print(self.annomode )
                imgwithann = self._addPeopleAnns(imgname, img)
            ...
            if saveimg:
                print("{}".format(util.custombasename(imgname) + '.jpg'))
                cv2.imwrite(os.path.join(savedir, util.custombasename(imgname) + '.jpg'), imgwithann)
                print("{}".format(os.path.join(savedir, util.custombasename(imgname) + '.jpg')))
            # cv2.putText(img, 'Press any button to continue', (0, 25), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
    ...
    def _addPersonAnns(self, imgname, img):
        objlist = self.annos[imgname]#[{'ignore': True, 'cate': 'fake person', 'rect': (651, 215, 655, 221)},
        print(imgname,objlist )
    ...
    def _addVehicleAnns(self, imgname, img):
    ...
    def _addPersonVehicleAnns(self, imgname, img):
        ...
        for objdict in vehicleobjlist:
            ...
            if objdict['ignore']:
                ...
                if self.showcate:
                    cv2.putText(img, cate, (xmin, ymin), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (b, g, r), 1)
        return img
    def _addCrowdAnns(self, imgname, img):
        objlist = self.annos[imgname]
        print(imgname,objlist )
        for objdict in objlist:
            cate = objdict['cate']
            if objdict['ignore']:
                ...
                if self.showcate:
                    cv2.putText(img, cate, (xmin, ymin), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (b, g, r), 1)
        return img
    def _addPeopleAnns(self, imgname, img):
        objlist = self.annos[imgname]
        print(imgname,objlist )
        for objdict in objlist:
            cate = objdict['cate']
