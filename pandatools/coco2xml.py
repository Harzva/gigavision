...
im_ext = 'jpg'
COCO_images = '/root/data/gvision/dataset/mot/12/train'
Json_addr = '/root/data/gvision/dataset/mot/12/annotations/instances_train.json'
im_num = 0
ob_count = 0
im_pairs = dict()
...
xml_dir = os.path.join("/root/data/gvision/dataset/mot/12/annotations", 'annotations_xml')  
# print(xml_dir)
# exit() 
...
print('len(annotation_list["annotations"])',len(annotation_list["annotations"]))
for i in range(0, len(annotation_list["annotations"])):
    ...
    if category_id in id_list:
        ...
        # print("dddddddddd",os.path.join(im_dir, str(im_pairs.get(im_name, 'None')) + '.' + im_ext))
        if os.path.exists(os.path.join(im_dir, str(im_pairs.get(im_name, 'None')) + '.' + im_ext)):
            #     # ---add object to imnum.xml---

                # read the xml root
            tree = ET.parse(os.path.join(xml_dir, str(im_name) + '.xml'))
            root = tree.getroot()
            ...
            print( "create new object element")
            ...
            print('s',im_name,xml_dir)
            print(os.path.join(xml_dir, str(im_name)))
            with open(os.path.join(xml_dir, str(im_name) + '.xml'), 'wb') as output:
                output.write(xml_str)


            print('--- Added {} to '.format(cat_name), str(im_name) + '.xml' ' ---')

        else:

            print("Copy image im_num.jpg ")
