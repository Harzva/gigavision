...
class DetResMerge():
    ...
    def keep_dets(self,dets,UP_boundary,temp):
        ...
        # print("same"*8)
        # for i in dets:
        #     _,left,up,scale,_,_,_,_,_,_,_ = i
        right = left + int(self.subimg_width/scale)
        down = up + int(self.subimg_height/scale)
        print("left,up,right,down,scale,UP_boundary",left,up,right,down,scale,UP_boundary)
        if up == UP_boundary:
        ...
        for (filename, objlist) in self.results.items():
            ...
            # keep_input.append([[srcimageid,int(left), int(up),float(scale),i['bbox'][0],i['bbox'][1],i['bbox'][2],i['bbox'][3],i['score'],i['image_id'],i['category_id']] for i in objlist])
                ...
                if 391<=srcimageid<=420:#14otcUP_boundary
                    keep_dets,_keep_values=self.keep_dets(np.array(keep_input),UP_boundarys[0],PANDA_TEST_SIZE[0])
                if 421<=srcimageid<=450:#15 nanshangongyuan
                    # print("srcimageid",srcimageid,PANDA_TEST_SIZE[1])
                    # print(np.array(keep_input))
                    # print(UP_boundarys[1])
                    keep_dets,_keep_values=self.keep_dets(np.array(keep_input),UP_boundarys[1],PANDA_TEST_SIZE[1])
                if 451<=srcimageid<=465:#16xiaoxue----------01
                    keep_dets,_keep_values=self.keep_dets(np.array(keep_input),UP_boundarys[2],PANDA_TEST_SIZE[2])
                if 466<=srcimageid<=480:#16xiaoxue--------02
                    keep_dets,_keep_values=self.keep_dets(np.array(keep_input),UP_boundarys[3],PANDA_TEST_SIZE[2])
                if 481<=srcimageid<=510:#17zhongguan
                    keep_dets,_keep_values=self.keep_dets(np.array(keep_input),UP_boundarys[4],PANDA_TEST_SIZE[3])
                if 511<=srcimageid<=540:#18xilin-------01
                    keep_dets,_keep_values=self.keep_dets(np.array(keep_input),UP_boundarys[5],PANDA_TEST_SIZE[4])
                if 541<=srcimageid<=555:#18xilin----------02
                    keep_dets,_keep_values=self.keep_dets(np.array(keep_input),UP_boundarys[6],PANDA_TEST_SIZE[5])
                if keep_dets is None:
                    continue
...
def full():
    ...
    # resfile="/root/data/gvision/mmdetection-master/workdir/detectors_123_fafaxue_resume/output/detectors_123resume_fafaxue_bbox.json"
    ...
    # resfile="/root/data/EfficientDet/output/fullbody_fafaxue_all.json"
    # splitannofile='/root/data/rubzz/ruby/ruby_output/test/person/split_test_method2_bigimageto1536_2.json'
    
    # resfile="/root/data/rubzz/ruby/post_processing/test_output/detectors_123_fafaxue_resume_7_bbox.json"
    # resfile="/root/data/rubzz/ruby/post_processing/detectors_1_2_fafaxue_3_bbox.json"
    # resfile="/root/data/rubzz/ruby/post_processing/test_output/detectors_3_crowdhuman_method2_resume_9_bbox.json"
    # splitannofile="/root/data/rubzz/ruby/ruby_output3/split_test_person_fafaxue_new/test_person_fafaxue_new.json"
    # splitannofile="/root/data/rubzz/ruby/ruby_output2/test_person_unsure_cell.json"

    # resfile="/root/data/EfficientDet/output/efficident_bigcar.json"
    # splitannofile='/root/data/rubzz/ruby/ruby_output/test/vehicle/method3_bbox_big_car_141518+noresize17.json'

    resfile="/root/data/gvision/mmdetection-master/workdir/detectors_1_3_fafaxue_data/test.bbox.json"
    # splitannofile='/root/data/rubzz/ruby/ruby_output/test/vehicle/method3_bbox_small_else_noresize.json'
    # resfile="/root/data/gvision/detectron2-master/workdir/output/my_end_ms_panda_fafaxue3/my_predict/final_nms0.5_fs0.05_all.json"
    # splitannofile='/root/data/rubzz/ruby/ruby_output3/split_test_person_fafaxue/test_person_fafaxue.json'
    splitannofile="/root/data/rubzz/ruby/ruby_output3/split_test_person_fafaxue_new/test_person_fafaxue_new.json"
    ...
    if test:
        ...
        outpath=f"/root/data/gvision/final_merge/{cls_}/coco_results/{network}_{dataset}"
    ...
    merge.mergeResults(outfile=outfile,is_nms=is_nms,nms_thresh=nms_thresh,nms_name=nms_name)
