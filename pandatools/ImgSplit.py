...
class ImgSplit():
    def __init__(self,
        ...
        self.slidewidth = self.subwidth - self.gap_w
        self.slideheight = self.subheight - self.gap_h
        self.thresh = thresh
        self.imagepath = os.path.join(self.basepath, addmode)
        self.annopath = os.path.join(self.basepath, 'image_annos', annofile)
        self.outimagepath = os.path.join(self.outpath, addmode)
    ...
    def splitdata(self, scale,imgrequest=None, imgfilters=[]):
        ...
        """
        with open(os.path.join(self.outpath, "readme.txt"), 'w') as f:
            f.write("scale:{}\nthresh:{}\nscene:{}\nsubwh{}\ngapwh{}".format("0.3",self.thresh,imgfilters,(self.subwidth,self.subheight),(self.gap_w,self.gap_h)))
        ...
        for imgname in imgnames:
            ...
            for imgfilter in imgfilters:
                if imgfilter in imgname:
                    # print("imgfilter",imgfilter,"imgname",imgname)
                    iskeep = True
    ...
    def SplitSingle(self, imgname, scale):
        ...
        objlist = imagedict['objects list']####################3333333333333
        ...
        while left < imgwidth:
            ...
            while up < imgheight:
                ...
                if self.saveimage:#########ssssssssssssssssssssssssssssssssssssssssssss
                    self.savesubimage(resizeimg, subimgname, coordinates)
                """keneng test——imgsplit"""
                # split annotations according to annotation mode###############222222222222222
                ...
                # height,width=cv2.imread(os.path.join(self.outpath,"image_train",subimgname)).shape[0:2]
                subimageannos[subimgname] = {
                    "image size": {
                        "height": down - up + 1,
                        "width": right - left + 1
                    },
                    "objects list": newobjlist############111111111111111111
                }
                if up + self.subheight >= imgheight:
                    break
                else:
        ...
        if xmax <= left or right <= xmin or ymax <= up or down <= ymin:
            ...
            """
             # and中含0，返回0； 均为非0时，返回后一个值， 
                ...
                0 or 1   # 返回1 
            """
        return intersection and intersection / (square + 1e-5) > self.thresh#有了and当intersection=0时返回0
    ...
    def restrainRect(self, rectdict, imgwidth, imgheight, coordinates):
        ...
        return {
            'tl': {
                'x': (xmin - left) / (right - left),#(xmin - left) / (right - lef+1),
                'y': (ymin - up) / (down - up)
            },
            'br': {
    ...
    def personAnnoSplit(self, objlist, imgwidth, imgheight, coordinates):
        ...
        for object_dict in objlist:
            ...
            if objcate == 'person':
                ...
                # only keep a person whose 3 box all satisfy the requirement & 是位运算 与\ 是续行的 意思。
                ...
                # keep a person whose one of box all satisfy the requirement
                if self.judgeRect(fullrect, imgwidth, imgheight, coordinates) | \
                   self.judgeRect(visiblerect, imgwidth, imgheight, coordinates) | \
                   self.judgeRect(headrect, imgwidth, imgheight, coordinates):
                    newobjlist.append({
    ...
    def savesubimage(self, img, subimgname, coordinates):
        ...
        subimg=cv2.resize(subimg, (right - left + 1,down - up + 1), interpolation=cv2.INTER_CUBIC)
        ...
        while left < imgwidth:
            ...
            while up < imgheight:
                ...
                if self.annomode == 'person':
                    ...
                    newobjlist = self.headpointAnnoSplit(objlist, imgwidth, imgheight, coordinates)
                # height,width=cv2.imread(os.path.join(self.outimagepath,subimgname)).shape[0:2]
                subimageannos[subimgname] = {
                    "image size": {
                        "height": down - up + 1,
                        "width": right - left + 1
