...
def indexResults(reslist,annopath=""):
    ...
    return results 
def GetAnnotBoxLoc(rectange_list,AnotPath):#AnotPath VOC标注文件路径
...
def fliter(imageid,cat,templist):
    if cat==1 or cat==2 or cat==3:
        if 391<=imageid<=420:#14otc
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/gvision/dataset/xmls_del_back/person_del_back/IMG_14_25.xml")
        if 421<=imageid<=450:#15 nanshangongyuan
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/gvision/dataset/xmls_del_back/person_del_back/IMG_15_26.xml")
        if 451<=imageid<=465:#16xiaoxue----------01
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/gvision/dataset/xmls_del_back/person_del_back/IMG_16_01.xml")
        if 466<=imageid<=480:#16xiaoxue--------02
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/gvision/dataset/xmls_del_back/person_del_back/IMG_16_17.xml")
        if 481<=imageid<=510:#17zhongguan
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/gvision/dataset/xmls_del_back/person_del_back/IMG_17_25.xml")
        if 511<=imageid<=540:#18xilin-------01
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/gvision/dataset/xmls_del_back/person_del_back/IMG_18_09.xml")
        if 541<=imageid<=555:#18xilin----------02
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/gvision/dataset/xmls_del_back/person_del_back/IMG_18_40.xml")
    if cat==4:
        if 391<=imageid<=420:#14otc
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/gvision/dataset/xmls_del_back/vehicle_del_back/IMG_14_25.xml")
        if 421<=imageid<=450:#15 nanshangongyuan
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/gvision/dataset/xmls_del_back/vehicle_del_back/IMG_15_26.xml")
        if 451<=imageid<=465:#16xiaoxue----------01
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/gvision/dataset/xmls_del_back/vehicle_del_back/IMG_16_01.xml")
        if 466<=imageid<=480:#16xiaoxue--------02
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/gvision/dataset/xmls_del_back/vehicle_del_back/IMG_16_17.xml")
        if 481<=imageid<=510:#17zhongguan
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/gvision/dataset/xmls_del_back/vehicle_del_back/IMG_17_25.xml")
        if 511<=imageid<=540:#18xilin-------01
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/gvision/dataset/xmls_del_back/vehicle_del_back/IMG_18_09.xml")
        if 541<=imageid<=555:#18xilin----------02
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/gvision/dataset/xmls_del_back/vehicle_del_back/IMG_18_40.xml")
    return templist
...
def model_fusion( outpath="/root/data/gvision/my_merge/fusion_results",
    outfile="fafaxue_final_wnms_all.json",model_num=3):
    results_all=[[],[],[],[]]
    a=["14","15","16","17","18"]
    b=["15","16","17"]
    no=["no"]
    ...
    for idj,i in enumerate(zip(
                list(results_resolve(model_path="/root/data/gvision/final_merge/final2/det_results.json",weight=[1,1,1,0.9],
                selected1=a,selected2=a,selected3=a,selected4=["15","16","17"],model_id=0)),

                list(results_resolve(model_path="/root/data/gvision/final_merge/fusion_results/final_nms_all.json",weight=[0.9,0.9,0.8,0.9],
                selected1=no,selected2=no,selected3=no,selected4=a,model_id=1)),

                list(results_resolve(model_path="/root/data/gvision/final_merge/tour.json",weight=[1,1,1,1],
                selected1=a,selected2=a,selected3=a,selected4=a,model_id=2)),
                )):
        a=[]
...
def main():
    ...
    if test:
        ...
        outpath="/root/data/gvision/final_merge/final3"
    model_fusion(outpath=outpath,
            outfile="fusion_all.json")#must all
