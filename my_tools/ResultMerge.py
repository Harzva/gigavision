...
class DetResMerge():
    def __init__(self,
        ...
        # self.respath = os.path.join(self.basepath, 'image_annos', resfile)
        ...
        self.indexResults()
        # if not os.path.exists(self.npyname):
        #     print("npy not exists --> run indexResults()")
        #     indexedresults=self.indexResults()
        #     print(indexedresults)
        #     self.results = indexedresults
        # else:
        #     print(f"npy exists --> load {self.npyname}")
        #     self.results = np.load(self.npyname,allow_pickle=True)
    ...
    def indexResults(self):
        ...
        self.results = indexedresults
            ...
            # np.save(self.npyname,indexedresults )
            # return indexedresults
    def mergeResults(self,is_nms,nms_thresh,outfile):
        """
        ...
        for (filename, objlist) in self.results.items():
            ...
            print(srcimageid,scale,left,up,nms_thresh)
            # [{ "image_id": int, "category_id": int, "bbox": [bbox_left, bbox_top, bbox_width, bbox_height], "score": float }，
            # { "image_id": int, "category_id": int, "bbox": [bbox_left, bbox_top, bbox_width, bbox_height], "score": float }]
            for objdict in objlist:
                """xywh-xyxy"""
                mergedresults[srcimageid].append([*recttransfer(objdict['bbox'], float(scale), int(left), int(up)),objdict['score'], objdict['category_id']])
        if is_nms:
            print(f"start---------------nms{nms_thresh}")
