...
cfg.SOLVER.MAX_ITER =ITERS_IN_ONE_EPOCH *50 # 300 iterations seems good enough for this toy dataset; you may need to train longer for a practical dataset
...
cfg.MODEL.WEIGHTS=os.path.join(cfg.OUTPUT_DIR,"model_0032717.pth")
...
def predict_my(visual=True,model_weight="model_final.pth"):
    ...
    def say(iss):
        ...
        return coco_list_result,file_name
    executor = ThreadPoolExecutor(max_workers=100)
    func_var = [[file_name,dict_value] for file_name,dict_value in dataset_test_dicts.items()]
    for coco_list_result,file_name in executor.map(say,func_var):
