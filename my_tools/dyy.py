...
split_params=[ ##
    {'global_scale':-1,'params':[0.6,0.12,6052,1000,4000]},
    {'global_scale':0,'params':[1,0.8,0.2,5000,6800,600,1000,600,300,800,400,1500,3500]},
...
class Split_train_seq():
    ...
    def split_scene(self):
        ...
        img_paths = [path for path in img_paths if get_frame_id(path)%10==0]
        current_frame=0
    ...
    def annoSplit(self,current_frame,sub_frame_id,imgwidth, imgheight,coordinates,img, subimgname):
        split=False
        ...
        subimg = cv2.resize(subimg, (self.subwidth, self.subheight))
        tool_image = copy.deepcopy(img[0: 1800, 0: 1800])
            ...
            for track_dict in self.annos:
                ...
                for frame_dict in track_dict["frames"]:
                    ...
                    # process  this picture and anno
                        ...
                        tl = {"x": rectdict[0] / imgwidth, "y": rectdict[1] / imgheight}
                        br = {"x": (rectdict[0] + rectdict[2]) / imgwidth, "y": (rectdict[1] + rectdict[3]) / imgheight}
                        rect = {"tl": tl, "br": br}
                        xmin = rect['tl']['x'] * imgwidth
                        ymin = rect['tl']['y'] * imgheight
                        xmax = rect['br']['x'] * imgwidth
                        ymax = rect['br']['y'] * imgheight
                        flag = ((xmax <= left or right <= xmin) and (ymax <= up or down <= ymin))
                        ...
                        elif flag == 0 :   #and ((int(ymin)!=0)and(int(ymax)!=0)and(int(xmin)!=0)and(int(xmax)!=0))
                            rect = self.restrainRect(rect, imgwidth, imgheight,
                                                    coordinates)  # change rect size in subimg   没问题
                            x, y, w, h = self.RectDict2List(rect, self.subwidth, self.subheight, scale=1,
                                                            mode='tlwh')  # to mot
                            if w!=0 and h!=0:
                                subimg[int(y):int(y + h), int(x):int(x + w), :] = 0
                                subimg[int(y):int(y+h),int(x):int(x+w),:] = cv2.resize(tool_image, (int(x+w)-int(x),int(y+h)-int(y)))

        if split:
            address = os.path.join(self.outpath,subimgname)
            cv2.imwrite(address, subimg)
