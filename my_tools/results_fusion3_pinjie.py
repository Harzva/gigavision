...
def sceneid2list(selected):
    selected_dict={"14":list(range(391,421)),"15":list(range(421,451)),"16":list(range(451,481)),"17":list(range(481,511)),"18":list(range(511,556)),"no":list(range(1000,1200)),"18_2":list(range(511,541)),"18_1":list(range(541,556)),"16_1":list(range(451,466)),"16_2":list(range(466,481))}
    temp_list=[]
    for i in selected:
...
def indexResults(reslist,annopath=""):
    ...
    return results 
def GetAnnotBoxLoc(rectange_list,AnotPath,thresh):#AnotPath VOC标注文件路径
    tree = ET.ElementTree(file=AnotPath)  #打开文件，解析成一棵树型结构
    root = tree.getroot()#获取树型结构的根
    ObjectSet=root.findall('object')#找到文件中所有含有object关键字的地方，这些地方含有标注目标
    backlist=[]
    print(f'{rectange_list[0]["image_id"]}  forbid zone before {len(rectange_list)}')
    ...
    for a in rectange_list:
        ...
        if np.where(inter_xml<=thresh)[0].shape[0]==len(ObjectSet):#则没有与bbox相交的xmlforbidzone < param or ==0
            backlist.append(a)
        # else:
        #     print(np.where(inter_xml==0)[0].shape[0]==len(ObjectSet))
        #     print("del")
    print(f'{rectange_list[0]["image_id"]}  forbid zone after {len(backlist)}')
    return backlist#+templist#17_newzhongguan
    # return backlist,templist
#
def fliter(imageid,cat,templist):
    
    if cat==1 or cat==2 or cat==3:
        # print("cat,imageid",cat,imageid)
        if 391<=imageid<=420:#14otc
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/rubzz/dataset/xmls_del_back/person_del_back/IMG_14_25.xml",thresh=0.05)
        if 421<=imageid<=450:#15 nanshangongyuan
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/rubzz/dataset/xmls_del_back/person_del_back/IMG_15_26.xml",thresh=0.05)
        if 451<=imageid<=465:#16xiaoxue----------01
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/rubzz/dataset/xmls_del_back/person_del_back/IMG_16_01.xml",thresh=0.05)
        if 466<=imageid<=480:#16xiaoxue--------02
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/rubzz/dataset/xmls_del_back/person_del_back/IMG_16_17.xml",thresh=0.05)
        if 481<=imageid<=510:#17zhongguan
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/rubzz/dataset/xmls_del_back/person_del_back/IMG_17_25.xml",thresh=0.05)
        if 511<=imageid<=540:#18xilin-------01
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/rubzz/dataset/xmls_del_back/person_del_back/IMG_18_09.xml",thresh=0.05)
        if 541<=imageid<=555:#18xilin----------02
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/rubzz/dataset/xmls_del_back/person_del_back/IMG_18_40.xml",thresh=0.05)
    if cat==4:
        # print("cat,imageid",cat,imageid)
        if 391<=imageid<=420:#14otc
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/rubzz/dataset/xmls_del_back/vehicle_del_back/IMG_14_25.xml",thresh=0.05)
        if 421<=imageid<=450:#15 nanshangongyuan
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/rubzz/dataset/xmls_del_back/vehicle_del_back/IMG_15_26.xml",thresh=0.05)
        if 451<=imageid<=465:#16xiaoxue----------01
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/rubzz/dataset/xmls_del_back/vehicle_del_back/IMG_16_01.xml",thresh=0.05)
        if 466<=imageid<=480:#16xiaoxue--------02
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/rubzz/dataset/xmls_del_back/vehicle_del_back/IMG_16_17.xml",thresh=0.05)
        if 481<=imageid<=510:#17zhongguan
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/rubzz/dataset/xmls_del_back/vehicle_del_back/IMG_17_25.xml",thresh=0.05)
        if 511<=imageid<=540:#18xilin-------01
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/rubzz/dataset/xmls_del_back/vehicle_del_back/IMG_18_09.xml",thresh=0.05)
        if 541<=imageid<=555:#18xilin----------02
            templist=GetAnnotBoxLoc(templist,AnotPath="/root/data/rubzz/dataset/xmls_del_back/vehicle_del_back/IMG_18_40.xml",thresh=0.05)
    return templist
...
def model_fusion( outpath="/root/data/gvision/my_merge/fusion_results",
    outfile="fafaxue_final_wnms_all.json",model_num=1):
    results_all=[[],[],[],[]]
    all_s=["14","15","16","17","18"]
    b=["15","16","17"]
    no=["no"]
    ...
    #             list(results_resolve(model_path="/root/data/rubzz/fengwei/picture_proecessing/2day/submitnms.json",weight=[1,1,1,1],
    #             selected1=no,selected2=no,selected3=no,selected4=["14"],model_id=0)),

    #             list(results_resolve(model_path="/root/data/rubzz/fengwei/picture_proecessing/2day/15submitnms0.1+0.18.json",weight=[1,1,1,1],
    #             selected1=no,selected2=no,selected3=no,selected4=["15"],model_id=1)),

    #             list(results_resolve(model_path="/root/data/gvision/final_merge/final2/det_results.json",weight=[1,1,1,1],
    #             selected1=a,selected2=a,selected3=a,selected4=["16"],model_id=3)),

    #             list(results_resolve(model_path="/root/data/gvision/my_merge/finalsubmission/final2/vehicle.json",weight=[1,1,1,1],
    #             selected1=no,selected2=no,selected3=no,selected4=["17","18"],model_id=3)),

    #             )):

#     cid1 0 cid2 0 cid3 40080 cid4 0
# cid1 24170 cid2 0 cid3 0 cid4 0
# cid1 16115 cid2 17553 cid3 25699 cid4 0
# cid1 0 cid2 0 cid3 11355 cid4 0
# cid1 107064 cid2 73352 cid3 23057 cid4 102912
    # for idj,i in enumerate(zip(
    #             list(results_resolve(model_path="/root/data/gvision/final_merge/13/coco_results/cascade_new/cascade_delinter0isfliter113datasetnewsetnms0.6.json",weight=[1,1,1,1],
    #             selected1=no,selected2=no,selected3=["17","18_2"],selected4=no)),

    #             list(results_resolve(model_path="/root/data/gvision/final_merge/12/coco_results/detectors_reusme_fafaxue/detectors_reusme_delinter0isfliter112datasetfafaxuesetnms0.6.json",weight=[1,1,1,1],
    #             selected1=["17"],selected2=no,selected3=no,selected4=no)),

    #             list(results_resolve(model_path="/root/data/gvision/final_merge/123/coco_results/detectors_reusme_fafaxue/detectors_reusme_delinter0isfliter1123datasetfafaxuesetnms0.6.json",weight=[1,1,1,1],
    #             selected1=["16_2"],selected2=["16_2"],selected3=["14","18_1"],selected4=no)),

    #             list(results_resolve(model_path="/root/data/gvision/final_merge/head/coco_results/detectors_reusme_fafaxue/detectors_reusme_delinter0isfliter1headdatasetfafaxuenms0.6.json",weight=[1,1,1,1],
    #             selected1=no,selected2=no,selected3=["15","16_2"],selected4=no)),

    #             list(results_resolve(model_path="/root/data/gvision/final_merge/final2/det_results.json",weight=[1,1,1,1],
    #             selected1=["14","15","16_1","18"],selected2=["14","15","16_1","18","17"],selected3=["16_1"],selected4=all_s)),

    #             )):
    for idj,i in enumerate(zip(
                list(results_resolve(model_path="/root/data/gvision/final_merge/final6/det_results.json",weight=[1,1,1,1],
                selected1=all_s,selected2=all_s,selected3=all_s,selected4=all_s)))):
        a=[]
    ...
    afternms_results1=wnms(results1,outpath,outfile=outfile,iouthresh=0.6,savejson=0,nmsname="setnms")#"softnms")#

    afternms_results2=wnms(results2,outpath,outfile=outfile,iouthresh=0.6,savejson=0,nmsname="setnms")#"softnms")#

    afternms_results3=wnms(results3,outpath,outfile=outfile,iouthresh=0.6,savejson=0,nmsname="setnms")#"nms")#

    afternms_results4=wnms(results4,outpath,outfile=outfile,iouthresh=0.6,savejson=0,nmsname="setnms")#"nms")#
...
def main():
    ...
    model_fusion(outpath=outpath,
            outfile="det_results_xml.json")#must all
