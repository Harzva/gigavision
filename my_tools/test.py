...
def _create_text_labels(classes, class_names,scores=None):
    """
    Args:
...
def register_ballon(balloon_visual=False):
    for d in ["train", "val"]:
    ...
    if balloon_visual:
        ...
        for d in random.sample(dataset_dicts, 3):
            ...
            print(os.path.join(cfg.OUTPUT_DIR,"my_split_visual","vis_{}".format(file_name)))
            ...
            cv2.imwrite(os.path.join(cfg.OUTPUT_DIR,"my_split_visual","vis_{}".format(file_name)),vis.get_image()[:, :, ::-1])
...
#train
train_impath="/root/data/gvision/dataset/train/train_and/s0.5_t0.8_all/image_train"
...
pv_center_split_image_path="/root/data/gvision/dataset/train/train_center/image_train"
PREDEFINED_SPLITS_DATASET = {
    ...
    "person_train": ("/root/data/gvision/dataset/train/train_and/s0.5_t0.8_all/image_annos/coco_person_train_hwnoi.json",train_impath),
    "vehicle_train": ("/root/data/gvision/dataset/train/train_and/s0.5_t0.8_all/image_annos/coco_vehicle_train_split_hwnoi.json",train_impath),
    "pv_train": ("/root/data/gvision/dataset/train/train_or/s0.3_t0.7_all/image_annos/coco_pv_train_bbox_hwnoi.json",
                "/root/data/gvision/dataset/train/train_or/s0.3_t0.7_all/image_train"),
    ...
    "pv_center": ("/root/data/gvision/dataset/train/train_center/image_annos/split_pv.json",pv_center_split_image_path),
...
cfg.OUTPUT_DIR="/root/data/gvision/detectron2-master/workdir/output/test"
# cfg.OUTPUT_DIR="/root/data/gvision/dataset/d2_output/my_pv_center_mask"
# cfg.MODEL.WEIGHTS=os.path.join(cfg.OUTPUT_DIR,"model_0018499.pth")

# cfg.DATASETS.TRAIN = ("balloon_train",)
cfg.DATASETS.TRAIN=("pv_center",) 
...
with open(os.path.join(cfg.OUTPUT_DIR,"config_train.yaml"),'w') as f:    #设置文件对象
    f.write("{}".format(cfg))  
# cfg.freeze()
...
def predict_ballon(): 
    ...
    with open(os.path.join(cfg.OUTPUT_DIR,"config_pre.yaml"),'w') as f:    #设置文件对象
        f.write("{}".format(cfg))  
    balloon_metadata= MetadataCatalog.get("balloon_train")#Metadata(name='balloon_train', thing_classes=['balloon'])
    print(balloon_metadata.get("thing_classes", None))#['balloon']
    print(type(balloon_metadata))#<class 'detectron2.data.catalog.Metadata'>
    # print(balloon_metadata.stuff_classes)
    # print(balloon_metadata.thing_colors)ColorMode.SEGMENTATION
    dataset_dicts = get_balloon_dicts("/root/data/gvision/dataset/raw_data/ballon/val")
    predictor = DefaultPredictor(cfg)
    for d in random.sample(dataset_dicts,1):    
        file_name=os.path.basename(d["file_name"])
        print(os.path.join(cfg.OUTPUT_DIR,"my_predict_split_visual","vis_train{}".format(file_name)))
        im = cv2.imread(d["file_name"])
        outputs = predictor(im)
        v = Visualizer(im[:, :, ::-1],metadata=balloon_metadata, scale=1,instance_mode=ColorMode.IMAGE_BW)    # remove the colors of unsegmented pixels)
        v = v.draw_instance_predictions(outputs["instances"].to("cpu"))
        os.makedirs(os.path.join(cfg.OUTPUT_DIR,"my_predict_split_visual"),exist_ok=True)
        cv2.imwrite(os.path.join(cfg.OUTPUT_DIR,"my_predict_split_visual","vis_1{}".format(file_name)),v.get_image()[:, :, ::-1])
...
def predict_my(visual=True,model_weight="model_final.pth"):
    ...
    for j,(file_name,dict_value) in  enumerate(dataset_test_dicts.items()):
        ...
        if visual and up=="2048" and image_id=="01":
            ...
            v = Visualizer(img[:, :, ::-1],metadata=train_dicts_metadata, scale=1,instance_mode=ColorMode.IMAGE_BW)# ColorMode.SEGMENTATION or ColorMode.IMAGE_BW) 
            v = v.draw_instance_predictions(pre_instances.to("cpu"))#draw xyxy
            os.makedirs(os.path.join(cfg.OUTPUT_DIR,"d2_predict_split_visual_all"),exist_ok=True)
            cv2.imwrite(os.path.join(cfg.OUTPUT_DIR,"d2_predict_split_visual_all","visual{}_{}".format(model_weight[6:-4],file_name)),v.get_image()[:, :, ::-1])
    print(os.path.join(cfg.OUTPUT_DIR, "my_predict",f"{model_weight[6:-4]}_predict_all.json"))
    f1=open(os.path.join(cfg.OUTPUT_DIR, "my_predict",f"{model_weight[6:-4]}_predict_all.json"),'w') 
    f1.write(json.dumps(coco_list_results,cls=MyEncoder))
    print("predict----------------end")
...
def merge():
    print("--------->>>>>>>>>merge-------------start")
    merge =ResultMerge.DetResMerge(resfile=os.path.join(cfg.OUTPUT_DIR, "my_predict","0018499_predict_all.json"), 
...
def main():
    ...
    #     predict_my(visual=True,model_weight=i)
    # "merge"
    # merge()
    "-------------"
