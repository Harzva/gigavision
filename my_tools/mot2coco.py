...
class PANDA2COCO:
    ...
    '''
    def __init__(self,root='/root/data/gvision/dataset/mot/PANDA_split_new/train',mode='train'):
        self.mode=mode
        self.root=root                    
    ...
    def _categories(self, num_categories=1):   # num_categories 为总的类别数
        for i in range(0, num_categories):
            category = {}
            category['id'] = i+1
            category['name'] = 'person'             # 可根据实际需要修改
    ...
    def to_coco(self):
        ...
        if self.mode=='train':
            dataset_dir='/root/data/gvision/dataset/mot/PANDA_detect/train'
            ...
            # for i in tqdm(range(9)):
                ...
                gt_group_keys = [key for key in gt_group_keys if key%2==0]
    ...
    def save_coco_json(self, instance):
        os.makedirs('/root/data/gvision/dataset/mot/PANDA_split_new/train/annotations',exist_ok=True)
        if self.mode=='train':
            save_path='/root/data/gvision/dataset/mot/PANDA_split_new/train/annotations/instances_train.json'
        elif self.mode=='valid':
            save_path='/root/data/gvision/dataset/mot/PANDA_split_new/train/annotations/instances_valid.json'
        with open(save_path, 'w') as fp:
            json.dump(instance, fp, indent=1, separators=(',', ': '))

panda2coco=PANDA2COCO(mode="train")###train or valid
instance=panda2coco.to_coco()
